...
from  common.commonconfig import base_url,ip_address,mask
...
class Illegal_Gateway(unittest.TestCase):
    def setUp(self):

        self.testcaseinfo = TestCaseInfo(id=11, name="非法格式的默认网关输入时，提示信息检查 ")
    ...
    def test_illegal_gateway(self):
        ...
        try:
            ...
            wrongip = ['192.168.www.4', '192.!23.tt.com', '192.168.%$.#', '1 92.168.  34', '192.168.344.2',
                       '192.-168.3.2']
            for ip in wrongip:
                self.webpage.clear_gateway()
                self.webpage.set_gateway(ip)
                ...
                if self.webpage.is_gatewayerr_display():
                    ...
                    try:
                        self.assertEqual(errorinfo,"默认网关错误，请重新输入。")
